...
type dnsseeder struct {
	id         wire.FlokicoinNet // Magic number - Unique ID for this network. Sent in header of all messages
...
func isDuplicateSeeder(s *dnsseeder) (bool, error) {
	...
	for _, v := range config.seeders {
		if v.id == s.id {
			return true, fmt.Errorf("duplicate Magic id %d. Already loaded for %s so can not be used for %s", v.id, v.name, s.name)
		}
